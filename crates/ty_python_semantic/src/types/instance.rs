    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        match self.inner {
                ...
                ) =>
            {
                ...
                // Recursive self references are preserved as the original class-backed protocol
                // when cycle detection short-circuits nested materialization. Turning that shape
                // into a synthesized protocol would create a fresh wrapper layer every time it is
                // materialized again, so keep the stable class-backed representation instead.
